  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="爱好" prop="studentHobby">
        <el-select v-model="queryParams.studentHobby" placeholder="请选择爱好" clearable size="small">
    ...
    <el-table v-loading="loading" :data="studentList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="爱好" align="center" prop="studentHobby" :formatter="hobbyFormat" />
      <el-table-column label="性别" align="center" prop="studentSex" :formatter="sexFormat" />
      <el-table-column label="状态" align="center" >
        <template slot-scope="scope">
          <el-switch
            v-model="scope.row.studentStatus"
            active-value="0"
            inactive-value="1"
...
export default {
  ...
  created() {
    this.getList();
    this.getDicts("sys_student_hobby").then(response => {
  ...
  methods: {
    ...
    // 爱好字典翻译
    hobbyFormat(row, column) {
      return this.selectDictLabel(this.hobbyOptions, row.studentStatus);
    },
    // 性别字典翻译
    ...
    handleStatusChange(row) {
      let text = row.studentStatus === "0" ? "启用" : "停用";
      this.$confirm('确认要"' + text + '""' + row.studentName + '"用户吗?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function() {
        return changeStudentStatus(row.studentId, row.studentStatus);
      }).then(() => {
        this.msgSuccess(text + "成功");
      }).catch(function() {
        row.studentStatus = row.studentStatus === "0" ? "1" : "0";
      });
    },
    // 取消按钮
    cancel() {
